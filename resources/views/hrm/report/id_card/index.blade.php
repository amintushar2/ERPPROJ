...
@section('idc_content')
    ...
    <div class="card shadow-sm border-0 rounded-3">
        ...
        <div class="idc-card-header">
            ...
            <a href="{{ route('id-card.create') }}" class="btn btn-sm ms-auto idc-no-print"
        ...
        ══════════════════════════════════════ --}}
            ...
            <div class="idc-btn-row idc-no-print">
                ...
                {{-- RUN_REPORTS → ID_CARD_bangla_level_knit --}}
                ...
                {{-- PB_BANGLA_FRONT → ID_CARD_bangla_front --}}
                ...
                {{-- PB_BANGLA_SINGLE → ID_CARD_bangla --}}
                ...
                {{-- CAED_LEBEL → ID_CARD_Process_lebel --}}
                ...
                {{-- PUSH_BUTTON136 → emp_lebel range --}}
                <button class="idc-btn-report idc-btn-outline" data-type="emp_label" data-report="emp_lebel.rdf">
                    <i class="bi bi-tags-fill"></i> Emp Label (Range)
                </button>

                {{-- PUSH_BUTTON134 → ID_CARD_Process_lebel range --}}
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-0">
                    ...
                    <div class="p-2 border-bottom d-flex gap-2 align-items-center">
                        <input id="idcLovSearch" class="form-control form-control-sm"
                            placeholder="Search by emp no or name...">
                        <select id="idcLovSecFilter" class="form-select form-select-sm" style="max-width:190px">
                            <option value="">All sections</option>
                            @foreach ($sections as $sec)
                                <option value="{{ $sec->section_no }}">
                                    {{ $sec->section_no }}
                    ...
                    <table class="table table-sm table-hover mb-0">
                        <thead>
                            <tr>
                                <th style="width:36px">
                                    <input type="checkbox" id="idcLovChkAll" onchange="idcLovToggleAll(this.checked)">
                                </th>
                                <th>Emp No</th>
                                <th>Name</th>
                                <th>Designation</th>
                        ...
                        <tbody id="idcLovBody"></tbody>
                    </table>
                </div>
                <div class="modal-footer py-2 justify-content-between">
                    <span id="idcLovCount" class="text-muted small">0 selected</span>
                    <div class="d-flex gap-2">
                        <button class="btn btn-sm btn-outline-secondary" onclick="idcLovClearAll()">Clear All</button>
...
@push('scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // LOV search/filter listeners
            document.getElementById('idcLovSearch').addEventListener('input', idcRenderLovRows);
            document.getElementById('idcLovSecFilter').addEventListener('change', idcRenderLovRows);
        });
        ...
        function idcFetchInline(q) {
            ...
            idcInlineTimer = setTimeout(() => {
                $.get('{{ route('id-card.api.employeesKeyUp') }}', {
                    ...
                    <div class="idc-drop-item"
                         onmousedown="idcPickInline('${e.emp_no}')">
        ...
        document.addEventListener('click', e => {
            if (!e.target.closest('#idcTagBox') &&
                !e.target.closest('#idcInlineDrop')) idcHideInline();
        });
        ...
        function idcRenderLovRows() {
            const q = document.getElementById('idcLovSearch').value.toLowerCase();
            ...
            const hits = idcLovData.filter(e =>
                (!q || e.emp_no.toLowerCase().includes(q) ||
                    e.emp_name.toLowerCase().includes(q)) &&
                (!sec || e.section_no === sec)
            );

            document.getElementById('idcLovBody').innerHTML = hits.map(e => `
        <tr class="${idcChecked.has(e.emp_no) ? 'table-primary' : ''}"
            style="cursor:pointer"
            onclick="idcLovToggleRow('${e.emp_no}', this)">
            <td onclick="event.stopPropagation()">
                <input type="checkbox" class="idc-lov-chk"
                       ${idcChecked.has(e.emp_no) ? 'checked' : ''}
                       onchange="idcLovToggleRow('${e.emp_no}', this.closest('tr'))">
            </td>
            <td><code class="text-primary" style="font-size:.78rem">${e.emp_no}</code></td>
            <td>
                <div style="font-size:.82rem">${e.emp_name}</div>
                ${e.emp_name_bn
                    ? `<small class="text-muted bangla-text">${e.emp_name_bn}</small>`
                    : ''}
            </td>
            <td style="font-size:.78rem">${e.designation ?? ''}</td>
            <td style="font-size:.78rem">${e.section ?? ''}</td>
            <td style="font-size:.78rem;white-space:nowrap">${e.join_date ?? ''}</td>
            <td>
                <span class="badge bg-${typeColor[e.card_type] ?? 'secondary'}"
        ...
        function idcLovToggleAll(checked) {
            document.querySelectorAll('#idcLovBody tr').forEach(tr => {
                const no = tr.querySelector('code').textContent.trim();
                const chk = tr.querySelector('.idc-lov-chk');
                if (checked) {
                    idcChecked.add(no);
                    tr.classList.add('table-primary');
        ...
        function idcUpdateLovCount() {
            document.getElementById('idcLovCount').textContent =
                idcChecked.size + ' selected';
        }

        // ══════════════════════════════════════════════════
        //  RUN REPORT BUTTONS
        //  Uses window.open() — GET request — so browser
        //  opens the PDF directly in a new tab.
        //  No AJAX / no binary response handling needed.
        // ══════════════════════════════════════════════════
        $(document).on('click', '.idc-btn-report', function() {
            ...
            Swal.fire({
                ...
                // flush once more in case tag was not committed
                ...
                // Build GET query string — emp_nos as plain comma string
                const empNosStr = idcTags.join(','); // e.g. "1001,1002,1003"

                const params = new URLSearchParams();
                params.set('emp_nos', empNosStr);
                ...
                const url = '{{ route('id-card.print') }}' + '?' + params.toString();

                // Open PDF in new browser tab — controller streams PDF directly
